...
class XlsFile:
    ...
    It takes file_path's path, the coordinates and sheet_index of the excel file_path as arguments.
    The coordinates:  are needed for deciding the start and end places of the parsing.
            ...
            if row_name != "":
                ...
                for each_col in range(first_col, last_col + 1):
                    ...
                    if value_of_each_column == "ERROR":
                        return Notification.raise_error(message=f"Cell has invalid value. Make sure all cell have ...!")
                    row_vals.append(value_of_each_column)

                rows[row_name] = row_vals
                empty_row_index += 1
            else:
                error_message = f"{self.get_order(empty_row_index)} row is empty!" \
    ...
    def _parse_cols(self, worksheet):
        ...
        for each_col in range(first_col, last_col + 1):                     # Added 1 otherwise it isn't inclusive
            ...
            if col_name != "":
                ...
                error_message = f"{self.get_order(empty_col_index)} column is empty! " \
    ...
    def _pop_col_name(self, worksheet, colx, start_rowx):
        ...
        if name is self.row_names:
            error_message = f"Duplication detected in column names: '{name}'." \
    ...
    def _pop_row_val(worksheet, rowx, start_colx):
        ...
        if type(value) == int or float or None:
            ...
            error_message = f"Cell ({rowx}, {start_colx}) is not a number value." \
    ...
    def _pop_col_val(worksheet, colx, start_rowx):
        ...
        if type(value) == int or float or None:
            ...
            error_message = f"Cell ({start_rowx}, {colx}) is not a number value. " \
                            f"Make sure that all cells contain a number value."
            return Notification.raise_error(error_message)
