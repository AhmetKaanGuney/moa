...
rows = json.dumps(source_matrix.get_rows())
cols = json.dumps(source_matrix.get_cols())
matrix_as_json = json.dumps({"rows": rows, "cols": cols}, indent=4)
...
print("ADDED to db: ")
for row in cur.execute("SELECT matrix FROM matricies WHERE user_id=?", (user_id,)):
    print(row)
